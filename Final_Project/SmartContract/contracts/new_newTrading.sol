...
contract Trading_New is Initializable {
    ...
    modifier isListed(uint ID) {
        Listing memory listi = Listings[ID];
    ...
    function buyItem(uint listingID) public payable {
        Listing memory buyingItem = Listings[listingID];
        ...
        IERC721(buyingItem.nftAddress).safeTransferFrom(
            address(this),
            msg.sender,
            buyingItem.tokenID
        );
        ...
        Listing storage listi = p_Listing[msg.sender][itemIndex];
        listi.exists = false;
    }
    ...
    function cancelListing(uint listingID) public isListed(listingID) {
        ...
        //Listing storage cancelItem = pListings[msg.sender][listingID];
